...
#
# Purpose
#   Read a new external park boundary source, restrict it to North Carolina
#   using a buffered state boundary, compare it with the current Master Park
#   Boundary Dataset, exclude source polygons whose area overlaps the master
#   by at least 5 percent, append the remaining polygons as new parks,
#   preserve existing park_id values, assign 12 character park_id values only
#   to appended parks, and write a new dated master boundary file.
#
# New source
#   USDA Forest Service Common Names, National Forest Boundary
#   EDW/EDW_ForestCommonNames_01 (MapServer), layer 2
#   Layers 0, 1, and 2 of that map service are display scale variants of the
#   same feature class (S_USA.FSCommonNames), so the downloaded dataset already
#   corresponds to the National Forest Boundary layer.
#
# Field mapping
#   COMMONNAME becomes MA_NAME
#   OWNER is assigned a constant value because the source has no managing
#   agency field. All features are administered by the USDA Forest Service.
#
# Spatial restriction
#   The source is national in extent. Only polygons intersecting the North
#   Carolina state boundary buffered by nc_buffer_m are retained. Forest units
#   extending across state lines are optionally clipped to that buffer.
#
# Identification rule
...
# Set to a layer name if the source contains more than one layer.
# Leave as NA to read the default layer.
new_layer <- NA_character_

# Optional local North Carolina state boundary.
# Leave as NA to download the boundary with tigris.
nc_path <- NA_character_
...
src_name <- "USDA Forest Service Common Names"
creator <- "youngjaewon"
...
# The source carries no managing agency field.
owner_value <- "USDA Forest Service"

# NAD83 North Carolina, meters
crs_projected <- 32119

# Buffer applied to the North Carolina state boundary before selecting
# source polygons, in meters.
nc_buffer_m <- 5000

# If TRUE, retained polygons are clipped to the buffered state boundary so
# that out of state portions of cross border forest units are not carried
# into the master. If FALSE, whole polygons are retained.
clip_to_nc <- TRUE
...
source_layers <- st_layers(new_path)

cat("\nLayers available in the new source\n")
...
if (is.na(new_layer)) {
  ...
  if (!new_layer %in% source_layers$name) {
    
    stop(
      "The requested layer is not present in the new source: ",
      new_layer
    )
  }
...
message(
  "Reading North Carolina state boundary"
)

if (is.na(nc_path)) {
  
  nc_state <- tigris::states(
    cb = TRUE,
    progress_bar = FALSE
  ) |>
    filter(STUSPS == "NC")
...
if (nrow(nc_state) == 0) {
  stop("The North Carolina state boundary could not be resolved.")
}
...
if (is.na(st_crs(nc_state))) {
  stop("The North Carolina state boundary does not have a CRS.")
}
...
# =============================================================================
# PART 2
# RESTRICT THE NEW SOURCE TO NORTH CAROLINA
# =============================================================================

nc_buffer <- nc_state |>
  st_geometry() |>
  st_union() |>
  st_buffer(nc_buffer_m) |>
  st_make_valid()
...
if (nrow(new) == 0) {
  stop("No source polygons fall within the buffered North Carolina boundary.")
}
...
cat(
  "\nNational source features: ",
  new_national_n,
  "\n",
  "Within North Carolina buffered by ",
  nc_buffer_m,
  " m: ",
  nrow(new),
  "\n",
  "Clipped to the buffered boundary: ",
  clip_to_nc,
  "\n",
  sep = ""
)
...
cat(
  "\nNew source features in North Carolina: ",
  nrow(new),
  "\n",
...
if (run_visual_check) {
  ...
  print(
    tm_shape(nc_buffer_sf) +
      tm_borders(
        col = "#212121",
        lwd = 1.5,
        group = "NC boundary buffer",
        group.control = "check"
      ) +
...
new_nonint_renamed <- new_nonint |>
  ...
  mutate(
    ...
    OWNER = standardize_owner_name(owner_value)
  )
...
message(
  "Source features within the buffered North Carolina boundary: ",
  nrow(new)
)
